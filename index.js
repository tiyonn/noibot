...
const bot = new Aoi.Bot({
	...
	autoUpdate: true,
	supressAll: true,
errorMessage: `$customEmoji[noibot_unlem]│Bir hata oluştu. Lütfen bunu bildirin. $log[Bir hata oldu. Hata: $error]`
});
...
code: `$log[aBilere sElam ćaTismayA deWamkEe`
})
//sa as
bot.command({
...
bot.command({
	name: 'saas-aç',
	code: `
 Başarılı! Artık biri **sa** ve benzeri mesajlar yazdığında yanıt vereceğim. Kapatmak için \`$getServerVar[prefix]saas-kapat\` yazabilirsin!
  $resetServerVar[saas]
...
bot.command({
	name: 'saas-kapat',
	code: `
  Başarılı! Artık biri **sa** ya da benzeri mesajlar yazdığı zaman yanıt vermeyeceğim. Tekrar açmak için \`$getServerVar[prefix]saas-aç\` yazabilirsin!
...
bot.command({
  ...
  $description[
    Prefixim: $getServerVar[prefix]
    Yardım komutum: $getServerVar[prefix]yardım
    İstatistik komutum: $getServerVar[prefix]i
    Davet komutum: $getServerVar[prefix]davet
...
bot.command({
    ...
    code:`
    $title[Başarılı!]
    $description[Artık prefixim $message! Bu prefixi kullanarak komutlarımı kullanabilirsiniz.]
    $footer[Noibot daima hizmetinizde!]
    $color[B89AFF]
    $argsCheck[1;Prefix değiştirmek için bir değer gir!]
    $setServerVar[prefix;$message]
    $onlyPerms[admin;{title:Hata!}{description:Bu işlem için yetkin yetersiz!}{color:RED}]`
  })
...
$color[GREEN]
  ...
  $onlyIf[$message!=;{title:Hata!} {description:$customEmoji[noibot_carpi]│Kanal belirtmen lazım! Örnek: \`$getServerVar[prefix]modlog-aç #modlog\`}{footer:Noibot daima hizmetinizde!}]
